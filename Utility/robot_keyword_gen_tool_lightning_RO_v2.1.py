...
def Connect():
    ...
    if status == 200 and session_id is not None:
        # sf_type = Radiobutton(top_frame, bg='sky blue', text="Classic", variable=var, value=1,
        #             command=s_type, fg='black').grid(row=6, column=1)
        # sf_type = Radiobutton(top_frame, bg='sky blue', text="Lightning", variable=var, value=2, command=s_type,
...
# def s_type(salesforce_type=None):
#     if var.get() == 1:
#         salesforce_type = 'classic'
#     elif var.get() == 2:
#         salesforce_type = 'lightning'
#     return salesforce_type
