...
class keyword_generator():
    ...
    def generateXpath(self, object, sf, sf_type):
        ...
        for layout in objectlayouts:
            labels = obj.describe_layout('012m00000000mdLAAQ')[layout]
            ...
            def generateXpath_detail_edit_layouts(object, header, labels, xpath=None):
                ...
                for objtype in iterlist:
                    if objtype != '':
                        o = object.replace(' ', '_')
                        h = header.replace(' ', '_')
                        l = objtype.replace('.', '').replace('/', '_').replace('(', '').replace(')', '').replace('?', '').replace(':', '').replace('-', '').replace(' ', '_').replace('#', '')
                                                            ...
                                                            if isinstance(k, OrderedDict):
                                                                for kd, kv in k.iteritems():
                                                                    if kd == 'details':
                                                                        ...
                                                                        if 'edit' in layout:
                                                                            csvdata = detailLabel.replace('.', '').replace('/', '_').replace('(', '').replace(')', '').replace('?', '').replace(':', '').replace('-', '').replace(' ', '_').replace('#', '')
                                                                    ...
                                                                    if kd == 'components':
                                                                        for kdata in kv:
                                                                            ...
                                                                            if 'edit' in layout:
                                                                                csvdata = compLabel.replace('.', '').replace('/', '_').replace('(', '').replace(')', '').replace('?', '').replace(':', '').replace('-', '').replace(' ', '_').replace('#', '')
                                                                                csv_label_list.append(csvdata)
